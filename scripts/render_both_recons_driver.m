% Driver: render JohnDoe2 + JohnDoe1 3-D recons via render_segmentation_recon.
...
for ci = 1:numel(cases)
    for vi = 1:numel(views)
        try
            fname = sprintf('%s_%s.png', cases(ci).name, views{vi});
            out_png = fullfile(out_dir, fname);
            fprintf('=> %s (%s)\n', cases(ci).name, views{vi});
            t0 = tic;
            render_segmentation_recon(cases(ci).mat, out_png, views{vi});
            fprintf('   done in %.1fs\n', toc(t0));
        catch ME
